...
export class ReturnsService {
  ...
  async create(userId: string, dto: CreateReturnDto): Promise<ReturnRequestDetail> {
    ...
    if (order.userId !== userId) {
      throw new ForbiddenException('Không có quyền trả hàng đơn này');
    }
    if (order.status !== OrderStatus.delivered) {
      throw new BadRequestException(
        'Chỉ trả hàng khi đơn đã giao thành công',
      );
    }
    ...
    if (existingPending) {
      throw new BadRequestException('Đơn hàng đã có yêu cầu trả hàng đang xử lý');
    }
    ...
    for (const item of dto.items) {
      ...
      if (!orderItem) {
        throw new BadRequestException(`Sách ${item.bookId} không có trong đơn`);
      }
      if (item.quantity < 1 || item.quantity > orderItem.quantity) {
        throw new BadRequestException(
          `Số lượng trả không hợp lệ cho sách ${item.bookId}`,
        );
      }
      returnItems.push({
        bookId: item.bookId,
    ...
    if (returnItems.length === 0) {
      throw new BadRequestException('Chọn ít nhất một sản phẩm trả hàng');
    }
  ...
  async findOne(id: string, user: PublicUser): Promise<ReturnRequestDetail> {
    ...
    if (!row) throw new NotFoundException('Yêu cầu trả hàng không tồn tại');
