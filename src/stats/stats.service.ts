...
export class StatsService {
  ...
  async getStaffDashboard(): Promise<DashboardStats> {
    const [books, orders, lowStock] = await Promise.all([
      this.prisma.book.count(),
      this.prisma.order.findMany({
        include: {
          items: true,
    ...
    return {
      ...
      recentOrders: orders.slice(0, 5).map((order) => {
        ...
        const { totalAmount: _, ...withoutAmount } = mapped;
        return withoutAmount as typeof mapped;
      }),
    };
  }
  ...
  async getReports(range: string): Promise<ReportsStats> {
    ...
    for (const o of currentOrders) {
      ...
      const cur = monthMap.get(sortKey) ?? {
        revenue: 0,
        orders: 0,
        label,
      };
